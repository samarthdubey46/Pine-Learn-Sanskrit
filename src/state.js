...
const ThemeProvider = (props) => {
    ...
    useEffect(() => {
        (async () => {
            ...
            if (parseInt(CurrentLevel) > 1 && parseInt(CurrentLevel) < 16) {
                const res = await Update(Token, pk, CurrentLevel)
             }
    ...
    useEffect(() => {
        (async () => {
            ...
            const Token = await getData_For_Each('token')
            // const streak = await getData_For_Each('streak')
            if (Profile_Pic !== undefined && String(Profile_Pic).length > 0 && Profile_Pic !== " ") {
                if (Triggers_Of_Badge.map(item => item.badge).includes(Profile_Pic)) {
                    console.log("Profile Pic Changed")
                    const res = await Update(Token, pk, -1, Profile_Pic,true)
                }
            }
            changeLatestBadge(GetLatestBadge_With_Other(CurrentLevel))
        })()
