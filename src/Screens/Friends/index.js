...
const RenderImage = ({ item, CurrentLevel, bottom }) => {
    ...
    return (
        <Surface style={{ elevation: 0, borderRightWidth: 0, borderRightColor: 'rgba(0,0,0,.1)', borderLeftWidth: 1, borderLeftColor: 'rgba(0,0,0,.1)', justifyContent: 'flex-end' }}>
            ...
            <Image resizeMode="contain" style={{ height: 90, width: WIDTH / 3.2, marginRight: 15 }} source={{ uri: item.badge === "" ? " " : item.badge  }} />
        </Surface>
    )
}
    ...
    const RenderOne = ({ item, index, pk }) => {
        ...
        return (
            <View style={{ height: 90, marginHorizontal: 10, borderBottomColor: 'rgba(0,0,0,.1)', borderBottomWidth: 1 }}>
                {/* <Button title="asd" onPress={() =>  console.log(item)}/> */}
                <View style={{ flexDirection: 'row', alignItems: 'center', }}>
                    <Image resizeMode="contain" style={{ width: 60, height: 80, marginRight: 15 }} source={{ uri: GetLatestBadge_With_Other(CurrentLevel_Other, index).level === 2 ?  String(item.Profile_Pic).length <= 0 ? 'https://i.ibb.co/pwGkhw1/none.jpg' : item.Profile_Pic : GetLatestBadge_With_Other(CurrentLevel_Other, index).badge  }} />
                    <View style={{ marginTop: 8 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                            <Text style={{ fontSize: 16, marginRight: 15 }}>{username}</Text>
                            {parseInt(pk_other) === parseInt(pk) && (<MaterialIcons color="green" type="MaterialIcons" size={18} name="verified-user" />)}
                        </View>
